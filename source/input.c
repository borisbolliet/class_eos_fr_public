...
int input_read_parameters(
  ...
  class_call(parser_read_string(pfc,"MG/DE type",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
 ...
 class_call(parser_read_double(pfc,"Omega0_ds",&param4,&flag4,errmsg),
             errmsg,
             errmsg);
  ...
  */
  ...
  if (flag4 == _TRUE_){
    pba->Omega0_ds = param4;
    Omega_tot += pba->Omega0_ds;
  }
  ...
  else if (flag4 == _FALSE_) // Fill up with dark sector fluid
    pba->Omega0_ds = 1. - pba->Omega0_k - Omega_tot;
  ...
  sprintf(pop->root,"%s_%s_w=%s_cs2=%s_%s",pop->root,name2,name3,name4,name);
  sprintf(ppt->root,"%s_%s_w=%s_cs2=%s_%s",ppt->root,name2,name3,name4,name);
 }
  else {
  sprintf(pop->root,"%s_%s_w=%s_%s",pop->root,name2,name3,name);
  sprintf(ppt->root,"%s_%s_w=%s_%s",ppt->root,name2,name3,name);
   }
  /*
    pba->a_start_ds = 1.e-2;
    double lambda = 3.*(1.+pba->w_ds);

    //Start DS in MATTER domination
 ...
 double n_plus = (-7.+sqrt(7.*7.-4.*2.*(-3)))/(2.*2.);
    ...
    pba->f_ini_ds = pba->b_ds*C_ds*pow(pba->a_start_ds,n_plus)+f_part_ini;
    pba->f_prime_ini_ds = n_plus*pba->b_ds*C_ds*pow(pba->a_start_ds,n_plus)+f_part_prime_ini;
...
int input_default_params(
  ...
  ppt->ALPHA_ds = 1.e9;
  ppt->k_max_ds = 0.;
  ppt->start_perturbs_ds = _FALSE_;
  pba->has_loop_over_b_ds = 1;
  pba->b_min_ds =-10.;
  pba->b_max_ds =2.;
  pba->N_b_ds = 50;
    pba->m2_hi_ini = 0.;
